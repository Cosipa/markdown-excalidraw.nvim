...
describe("config", function()
  ...
  describe("setup", function()
    it("uses defaults when called with empty table", function()
      config.setup({})
      assert.equal("auto", config.get().theme)
      assert.is_true(config.get().auto_open)
    end)
    ...
    it("overrides single option", function()
      ...
      assert.equal("dark", config.get().theme)
    ...
    it("overrides multiple options", function()
      ...
      assert.equal("light", config.get().theme)
      assert.is_false(config.get().auto_open)
      assert.equal("debug", config.get().log_level)
    end)
    ...
    it("can override server port", function()
      ...
      assert.equal(8080, config.get().server_port)
    end)

    it("can set browser command", function()
      config.setup({ browser_command = "firefox" })
      assert.equal("firefox", config.get().browser_command)
    end)
    ...
    it("can set library_path", function()
      ...
      assert.equal("~/my-library.excalidrawlib", config.get().library_path)
    end)
