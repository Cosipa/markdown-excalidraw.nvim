...
system browser, with automatic file synchronization back to disk.

This plugin runs a lightweight local Python server that serves the Excalidraw
editor (through esm.sh) and handles reading/writing `.excalidraw` files via REST
API.
...
- Neovim >= 0.9
- Python 3 (using stdlib only)
- A modern web browser

## AI Usage Disclosure

This plugin was developed with the assistance of AI tools. The code has been
reviewed and tested.
...
1. When you open or create an `.excalidraw` file, the plugin starts a local
   Python HTTP server (if not already running).
2. The server serves a single-page Excalidraw editor loaded from
   [esm.sh](https://esm.sh) CDN.
3. Your default browser opens to
   `http://127.0.0.1:<port>/?file=<path>&library=<path>`.
4. As you draw, changes are automatically saved back to disk via the server's
   REST API (debounced, default 1s).
5. Library items are synced separately - changes to the library are saved to the
   configured `.excalidrawlib` file.
6. Closing the browser tab triggers a final save via `sendBeacon`.
7. The server automatically stops after the configured idle timeout (default 15
   min) or when Neovim exits.

## Library Support

The plugin supports a shared library of reusable shapes and stencils. Library
items are stored in an `.excalidrawlib` file.

### Configuration

Set `library_path` to point to your library file. If not set, defaults to
...
- On page load, the library is fetched from the server and loaded into
  Excalidraw via the `updateLibrary` API, bypassing browser localStorage.
- When you add or remove library items, changes are debounced and saved to disk
  automatically.
- A status indicator in the bottom-right corner shows "Library saving..." /
  "Library saved" feedback.
...
Place your cursor on a markdown link like `[diagram](./my-diagram.excalidraw)`
and run `:ExcalidrawOpen` to open that file directly.

### Creating linked diagrams

Run `:ExcalidrawCreate` to create a new diagram. A markdown link is
automatically inserted at your cursor position, making it easy to embed diagrams
in your notes.

## Features

- **Automatic file sync** - changes in the browser are debounced and saved to
  disk.
- **Library support** - shared shapes/stencils synced to `.excalidrawlib` file.
- **Last-chance save** - uses `navigator.sendBeacon` on tab close to avoid data
  loss.
- **Atomic writes** - writes to a temp file then renames, preventing data loss.
- **Server idle timeout** - server auto-stops after inactivity to free
  resources.
- **Security** - server binds to localhost only, path validation limits file
  access to the directory of the first opened file.
- **No browser caching** - `Cache-Control: no-store` headers ensure fresh data
  on every open.
...
- **Internet required** - The Excalidraw editor is loaded from a CDN (esm.sh).
  Offline mode requires bundling the assets locally, which is not the best idea.
